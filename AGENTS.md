...
5. Extraction 工作流
   ...
   - 段以连续 assistant 消息 run 的末条闭合，无 assistant 收尾的尾巴不
     消费。段（一轮＝触发消息＋回复 run）是不可分割的原子单位，切块不得
     从中截断。两种模式共用同一装包规则：从最新闭合段向旧并入整段成块，
     累计不超过 `extractionWindowMessages` 加其二分之一即并入同块——窗口
     是模糊预算而非硬上限，盈余不超过半窗不开新块，最新一轮不因回复多条
     被拆开或切走上下文；余量更大时作为前序块继续同规则提取，不丢消息。
     锚定模式（默认）下超出预算的段独立成块、不截断；旁听模式
     （`extractionIncludeOverheard`）下超出预算的超长段先切成不超过预算
     的子块（边界尽量落在 assistant 之后）再按同一规则装包，是段原子
     原则的唯一例外。冷启动游标取首个合规 after-chat 时点的日志末位，
     不重提取回填历史；回填失败待重试的会话顺延至重试成功后的首个
     after-chat。会话清理即时终止在途排干的后续消费。
