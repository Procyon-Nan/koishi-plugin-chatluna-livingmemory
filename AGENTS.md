...
5. Extraction 工作流
   ...
   - 段以连续 assistant 消息 run 的末条闭合，无 assistant 收尾的尾巴不
     消费。段（一轮＝触发消息＋回复 run）是不可分割的原子单位，任何切块
     不得从中截断。参与锚定模式（默认）从最新闭合段向旧并入整段成块：
     累计不超过 `extractionWindowMessages` 加其二分之一即并入同块——窗口
     是模糊预算而非硬上限，盈余不超过半窗不开新块，最新一轮不因回复多条
     被拆开或切走上下文；余量更大时作为前序块继续同规则提取；超出预算的
     段独立成块、不截断，不丢消息。旁听模式（`extractionIncludeOverheard`）
     下按窗口切分全部提取。冷启动游标取首个合规 after-chat 时点的日志
     末位，不重提取回填历史；回填失败待重试的会话顺延至重试成功后的
     首个 after-chat。会话清理即时终止在途排干的后续消费。
   - 单块失败连续 3 次后记录 failed job 并放弃该块、继续后续块（锚定模式
     积压超出单块预算时同样多块串行）；不足 3 次中断本次排干，连同新消息
     下次重试。不得重复处理已成功推进游标的消息，也不得在失败后错误丢弃
     未消费消息。
