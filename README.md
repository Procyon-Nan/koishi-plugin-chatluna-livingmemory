...
- 在 WebUI 中手动执行 Dream 流程会对指定预设的活跃记忆执行分区聚类和整理，完成后按配置更新用户画像。
- 自动 Dream 以尚未完成 consolidation 的记忆数量作为触发依据。阈值同时是单次任务的批次大小；每次只处理最早的一批，不会连续清空全部积压。
- 自动 Dream 先整理本批新增记忆，再将仍需处理的记忆逐条与已固化的活跃旧记忆进行 content 向量检索，每条最多读取 30 条候选。该流程不执行 HDBSCAN，也不生成用户画像。
...
默认情况下，只要预设启用了自动记忆提取，所有使用该预设的群聊和私聊都会产生记忆。开启 `开启自动记忆总结（提取）白名单` 后，只有白名单内的会话才会自动提取记忆。

白名单列表填写用户可直接获取的平台 id：群聊填群号，私聊填对方的用户 QQ 号。两种接入方式（ChatLuna 主插件与 Character）都使用同一份列表，判定所用的 id 与平台原始 id 一致，不需要填写 `conversationId` 或带前缀的频道 id。

关闭白名单时，白名单列表本身不生效。开启白名单但列表为空，等同于全局关闭自动记忆提取。

白名单只约束自动记忆提取。非白名单会话仍然照常召回记忆、注入记忆快照和用户画像，`living_memory_create_memory` 主动创建工具也不受白名单限制，只是不再由对话内容产生新的自动提取记忆。
