...
- pending: 修复 WebUI 快照详情弹窗命中记忆较多时超出部分被裁剪且无法滚动的问题。Console 宿主样式把弹窗容器改为居中 flex 列布局，超高的弹窗会被压缩到视口高度、再被弹窗自身的 overflow: hidden 裁剪；现对快照弹窗限高并让正文内部滚动，标题栏保持固定。同时修正该弹窗遮罩类名的笔误以恢复遮罩背景样式，并移除其样式文件中没有竞争规则的 !important。
